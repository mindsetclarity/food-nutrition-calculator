...
import 'piccolore';
import { v as createRenderInstruction, m as maybeRenderHead, h as addAttribute, s as spreadAttributes, p as renderSlot, k as renderTemplate, o as renderComponent, u as unescapeHTML } from './entrypoint_DdV4XD_W.mjs';
import 'clsx';
...
}, "C:/fnc/src/components/ui/Button.astro", void 0);
...
}, "C:/fnc/src/components/site/NavigationLink.astro", void 0);
...
Analyze a Recipe
` })} </div> </nav> </div> </header> ${renderScript($$result, "C:/fnc/src/components/site/Header.astro?astro&type=script&index=0&lang.ts")}`;
}, "C:/fnc/src/components/site/Header.astro", void 0);
...
}, "C:/fnc/src/components/site/Footer.astro", void 0);
...
}, "C:/fnc/src/components/seo/JsonLd.astro", void 0);

const $$SeoHead = createComponent(($$result, $$props, $$slots) => {
  const Astro2 = $$result.createAstro($$props, $$slots);
  Astro2.self = $$SeoHead;
  const { title, description, canonicalPath, noindex, ogType, ogImage, keywords, jsonLd } = Astro2.props;
  const seo = buildSeoMetadata({ title, description, canonicalPath, noindex, keywords });
  const og = buildOpenGraphMetadata({ title, description, ogType, ogImage });
  const twitter = buildTwitterMetadata({ title, description, ogImage });
  const canonical = canonicalPath ? buildCanonicalUrl(canonicalPath) : void 0;
  return renderTemplate`<!-- SEO Core --><title>${seo.title}</title><meta name="description"${addAttribute(seo.description, "content")}>${seo.keywords && renderTemplate`<meta name="keywords"${addAttribute(seo.keywords, "content")}>`}${canonical && renderTemplate`<link rel="canonical"${addAttribute(canonical, "href")}>`}${seo.noindex && renderTemplate`<meta name="robots" content="noindex, nofollow">`}${!seo.noindex && renderTemplate`<meta name="robots"${addAttribute(siteConfig.robots, "content")}>`}<!-- Open Graph --><meta property="og:title"${addAttribute(og.title, "content")}><meta property="og:description"${addAttribute(og.description, "content")}>${og.type && renderTemplate`<meta property="og:type"${addAttribute(og.type, "content")}>`}${og.image && renderTemplate`<meta property="og:image"${addAttribute(og.image, "content")}>`}${canonical && renderTemplate`<meta property="og:url"${addAttribute(canonical, "content")}>`}<meta property="og:site_name"${addAttribute(siteConfig.siteName, "content")}><meta property="og:locale"${addAttribute(siteConfig.locale, "content")}><!-- Twitter -->${renderTemplate`<meta name="twitter:card"${addAttribute(twitter.card, "content")}>`}<meta name="twitter:title"${addAttribute(twitter.title, "content")}><meta name="twitter:description"${addAttribute(twitter.description, "content")}>${twitter.image && renderTemplate`<meta name="twitter:image"${addAttribute(twitter.image, "content")}>`}<!-- Theme --><meta name="theme-color"${addAttribute(siteConfig.themeColor, "content")}>${jsonLd && renderTemplate`${renderComponent($$result, "JsonLd", $$JsonLd, { "schema": jsonLd })}`}`;
}, "C:/fnc/src/components/seo/SeoHead.astro", void 0);
