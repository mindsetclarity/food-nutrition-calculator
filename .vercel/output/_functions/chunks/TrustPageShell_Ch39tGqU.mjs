...
import 'piccolore';
import { m as maybeRenderHead, k as renderTemplate, o as renderComponent, p as renderSlot } from './entrypoint_DdV4XD_W.mjs';
...
}, "C:/fnc/src/components/trust/TrustHero.astro", void 0);
...
Learn Hub
` })} </div> </div> </section>`;
}, "C:/fnc/src/components/trust/TrustCTASection.astro", void 0);

const $$TrustPageShell = createComponent(($$result, $$props, $$slots) => {
  const Astro2 = $$result.createAstro($$props, $$slots);
  Astro2.self = $$TrustPageShell;
  const { title, description, eyebrow, badges = [], updatedAt, showCta = true } = Astro2.props;
  const jsonLd = buildWebPageSchema({
    name: title,
    description,
    url: `${siteConfig.siteUrl}${Astro2.url.pathname}`
  });
  return renderTemplate`${renderComponent($$result, "Layout", $$SiteLayout, { "title": title, "description": description, "jsonLd": jsonLd }, { "default": ($$result2) => renderTemplate` ${maybeRenderHead()}<main id="main-content" class="min-h-screen bg-warm-ivory pb-24"> ${renderComponent($$result2, "TrustHero", $$TrustHero, { "title": title, "description": description, "eyebrow": eyebrow, "badges": badges, "updatedAt": updatedAt })} <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12 lg:py-16"> <div class="prose prose-emerald prose-lg max-w-none text-ink-navy"> ${renderSlot($$result2, $$slots["default"])} </div> </div> ${showCta && renderTemplate`${renderComponent($$result2, "TrustCTASection", $$TrustCTASection, {})}`} </main> ` })}`;
}, "C:/fnc/src/components/trust/TrustPageShell.astro", void 0);
