...
export const deepseekProvider: LLMProvider = {
  ...
  generate: async (request: LLMRequest): Promise<LLMResponse> => {
    if (!import.meta.env.DEEPSEEK_API_KEY) {
      return {
        ok: false, provider: 'deepseek', model: getLLMModel('deepseek'), text: '', warnings: [],
        error: { code: LLM_ERRORS.NOT_CONFIGURED, message: "DeepSeek API key is missing", retryable: false }
      };
    }

    const model = request.model || getLLMModel('deepseek');
    const baseUrl = getLLMBaseUrl('deepseek');
    ...
    try {
      ...
      if (!res.ok) {
        return {
          ok: false, provider: 'deepseek', model, text: '', warnings: [],
      ...
      return { ok: true, provider: 'deepseek', model, text, warnings: [] };

    } catch (err: any) {
      if (err.name === 'AbortError') {
        return { ok: false, provider: 'deepseek', model, text: '', warnings: [], error: { code: LLM_ERRORS.TIMEOUT, message: "Request timed out", retryable: true } };
      }
      return { ok: false, provider: 'deepseek', model, text: '', warnings: [], error: { code: LLM_ERRORS.FAILED, message: "Network or fetch error", retryable: true } };
    }
  }
};
